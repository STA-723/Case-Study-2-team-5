...
lm_1 <- lmer(log_price ~ latitude + longitude + room_type +
...
lm_2 <- lmer(log_review ~ latitude + longitude + room_type +
                          calculated_host_listings_count + availability_365 + 
                          (1 | neighbourhood_group) + (1 | neighbourhood), data = dat,
                          REML=TRUE)
summary(lm_2)
anova(lm_2) # test fixed effect
rand(lm_2) # test random effect
plot(lm_2) # residual plot


lm_3 <- glmer(number_of_reviews ~ room_type + calculated_host_listings_count + availability_365 + 
                          (1 | neighbourhood_group) + (1 | neighbourhood), data = dat,
                          family = poisson)
```
...
termMatrix <- term_matrix(dat$name)
# how to use it?
```
...
```{r}
hist(dat$log_price) #4, 6
dat <- dat %>%
        mutate(price_ind = ifelse(log_price<4,"low",ifelse(log_price<6,"medium","high")))
table(dat$price_ind)
...
dat_test <- dat[-train_index, ]
# fit a random forest model (using ranger)
rf_fit <- train(price_ind ~ latitude + longitude + room_type + reviews_per_month +
                          calculated_host_listings_count + availability_365 +
                          neighbourhood_group + neighbourhood, data = dat_train,
                          method = "ranger")
rf_fit
