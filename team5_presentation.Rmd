...
library(cowplot)
p1 <- ggdraw() + draw_image("Boxplot_PriceBoroughs.png")
p2 <- ggdraw() + draw_image("Mosaic_PriceLevelBoroughs.png")
pp1<- plot_grid(p1, p2)
ggdraw(pp1)
```
...
airbnb_eda <- read.csv("./AB_NYC_2019.csv")
# chi-square test
airbnb_eda <- airbnb_eda %>% filter(last_review!="") %>% mutate(popularity_level = ifelse(reviews_per_month<=quantile(reviews_per_month, 0.25), "below Q1", ifelse(reviews_per_month<=quantile(reviews_per_month, 0.75), "between Q1 and Q3", "above Q3")))
airbnb_eda$popularity_level <- factor(airbnb_eda$popularity_level, levels = c("below Q1", "between Q1 and Q3", "above Q3"))

Popularity_Level_by_Boroughs <- table(airbnb_eda$popularity_level, airbnb_eda$neighbourhood_group)
chisq.test(Popularity_Level_by_Boroughs)
```

```{r message=FALSE, echo=FALSE}
p1 <- ggdraw() + draw_image("Boxplot_MonthlyReviewBoroughs.png")
p2 <- ggdraw() + draw_image("Mosaic_PopularityLevelBoroughs.png")
pp1<- plot_grid(p1, p2)
ggdraw(pp1)
```
...
airbnb_eda <- read.csv("./AB_NYC_2019.csv")
# chi-square test
RoomType_by_Boroughs <- table(airbnb_eda$room_type, airbnb_eda$neighbourhood_group)
chisq.test(RoomType_by_Boroughs)
```

```{r message=FALSE, echo=FALSE, out.width = '70%',fig.align = "center"}
p1 <- ggdraw() + draw_image("Mosaic_RoomTypeBoroughs.png")
pp1<- plot_grid(p1)
ggdraw(pp1)
```
...
p1 <- ggdraw() + draw_image("XGBoost_LogPrice.png")
pp1<- plot_grid(p1)
ggdraw(pp1)
```
...
```{r message=FALSE, echo=FALSE, out.width = '75%',fig.align = "center"}
p1 <- ggdraw() + draw_image("XGBoost_LogMonthlyReview.png")
pp1<- plot_grid(p1)
ggdraw(pp1)
```
...
```{r message=FALSE, echo=FALSE, out.width = '75%',fig.align = "center"}
p1 <- ggdraw() + draw_image("MonthlyReview_Price.png")
pp1<- plot_grid(p1)
ggdraw(pp1)
```
