...
### Log Review vs Last Review Year
```{r}
airbnb %>% filter(!is.na(last_review)) %>% ggplot(aes(x = log(price), y = log(reviews_per_month))) + geom_point()
airbnb %>% filter(!is.na(last_review)) %>% ggplot(aes(x = price, y = reviews_per_month)) + geom_point() +
  ggtitle("Monthly Review vs Price")
```
...
ggplot(airbnb[airbnb$reviews_per_month<=10 & airbnb$last_review!="",], aes(x=neighbourhood_group, y=reviews_per_month, fill=neighbourhood_group)) + geom_boxplot() + ggtitle("Reviews per Month by Boroughs (reviews <=10)") + theme(legend.position="bottom")
...
ggplot(data = airbnb %>% filter(last_review!="")) +
  geom_mosaic(aes(x = product(popularity_level, neighbourhood_group), fill=popularity_level), na.rm=TRUE) + labs(x="Boroughs", y="Popularity Level") + ggtitle("Popularity Level by Boroughs") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom")
```
