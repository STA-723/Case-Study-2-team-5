...
mod1 <- lmer(price ~ 1 + reviews_per_month + room_type + minimum_nights + 
         last_review + calculated_host_listings_count + aspec + availability_365 + 
         (1|neighbourhood_group/neighbourhood), data = data)
# 2. Monthly reviews on price
mod2 <- lmer(reviews_per_month ~ 1 + price + room_type + minimum_nights + 
         last_review + calculated_host_listings_count + aspec + availability_365 + 
         (1|neighbourhood_group/neighbourhood), data = data)
...
empirical_vs_review <- variog(coords = data[,c("longitude", "latitude")],
                       data = resid(mod2))
par(mfrow = c(1, 2))
plot(empirical_vs_price) 
plot(empirical_vs_review)
```
...
mod3 <- lmer(value ~ variable:(room_type + last_review + minimum_nights + 
...
```{r}
coefs_n <- coef(mod3)[[1]] # Neighbourhood fixed effects
coefs_b <- coef(mod3)[[2]] # Borough fixed effects
vs_n <- VarCorr(mod3)[[1]] # RE variance components
vs_b <- VarCorr(mod3)[[2]]
marginal_vs <- vs_n + vs_b + diag(rep(sigma(mod3)^2, 2))
intercepts_b_price <- (coefs_b[,grepl("variableprice", colnames(coefs_b))])[,1]
intercepts_n_price <- (coefs_n[,grepl("variableprice", colnames(coefs_n))])[,1]
intercepts_b_rev <- (coefs_b[,grepl("variablereviews_per_month", colnames(coefs_b))])[,1]
intercepts_n_rev <- (coefs_n[,grepl("variablereviews_per_month", colnames(coefs_n))])[,1]
```
