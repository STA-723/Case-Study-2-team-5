...
data <- data %>% 
    mutate(price = log(price), 
           ...
           aspec = ifelse(availability_365==0, 0, 1))
```
...
mod1 <- lmer(price ~ 1 + reviews_per_month + room_type + minimum_nights + 
         last_review + calculated_host_listings_count + aspec + 
...
```{r bv_lmer, cache=T}
data_for_bv <- data %>% 
  select(price, reviews_per_month, room_type, last_review, aspec, 
...
mod3 <- lmer(value ~ variable:(room_type + last_review + aspec)-1 + 
...
# Not sure if I'll eventually fit Stan?
# 
# 
# library(rstan)
