...
base_model <- lm(log(price) ~ minimum_nights + calculated_host_listings_count + 
               room_type + neighbourhood_group + availability_365, data = lm_data)
...
#
lm_data_m <- lm_data %>% filter(neighbourhood_group == "Manhattan")
lm_data_b <- lm_data %>% filter(neighbourhood_group == "Brooklyn")
r_m <- resid(lm(log(price) ~ minimum_nights + calculated_host_listings_count + room_type, data = lm_data_m))
r_b <- resid(lm(log(price) ~ minimum_nights + calculated_host_listings_count + room_type, data = lm_data_b))
v_m <- variog(data = r_m, coords = lm_data_m[,c("longitude", "latitude")])
v_b <- variog(data = r_b, coords = lm_data_b[,c("longitude", "latitude")])
par(mfrow = c(1,2)); plot(v_m); plot(v_b)
```

  - Dirichlet allocation (topic modeling) for strings
