...
words_tfdoc %>%
  ...
  geom_col() + labs(x="", y="Frequency",title="Words with highest frequency") +
  theme(axis.text.x = element_text(angle=45), 
        plot.title = element_text(hjust = 0.5))
ggsave("figures/tf.png")
```

```{r}
# LDA
# Removing some words that appear in all topics
words_to_remove <- c(words_tfdoc %>% filter(tf >= 0.05) %>% select(word) %>% unlist(), "apt", "br", "spacious")
words_lda <- words %>%
...
png("figures/wordcloud_manhattan.png", width=6, height=6, units='in', res = 400)
wc_neighborhood_asso("manhattan", max.words = 50)
dev.off()
```

```{r}
wc_neighborhood_asso("brooklyn", max.words = 50)
png("figures/wordcloud_queens.png", width=6, height=6, units='in', res = 400)
wc_neighborhood_asso("queens", max.words = 50)
dev.off()

png("figures/wordcloud_bronx.png", width=6, height=6, units='in', res = 400)
wc_neighborhood_asso("bronx", max.words = 50)
dev.off()
```
...
badnames <- airbnb %>%
  ...
  mutate(name_nospace = 1 * (!grepl(" ", name) & has_nonenglish == 0))
badnames %>% 
...
summary(lm(log(price) ~ has_nonenglish + name_nospace + neighbourhood_group + 
           ...
           data = badnames%>% filter(!is.na(last_review))
           ))
```

```{r}
summary(lm(log(reviews_per_month) ~ has_nonenglish + name_nospace + neighbourhood_group + 
...
data_for_bv <- airbnb %>%
  ...
  mutate(has_nonenglish = 1 * grepl("IS_NOT_ENGLISH", iconv(name, "latin1", "ASCII", sub="IS_NOT_ENGLISH"))) %>%  # remove non latin words
  mutate(name_nospace = 1 * (!grepl(" ", name) & has_nonenglish == 0)) %>% 
  select(-id, -name, -host_id, -host_name, -latitude, -longitude) %>% 
...
ggsave(filename = "figures/coef_price_name_all.png", plot = g1, device = "png", width = 7, height = 5, units = "in")
ggsave(filename = "figures/coef_review_name_all.png", plot = g2, device = "png", width = 7, height = 5, units = "in")
```
